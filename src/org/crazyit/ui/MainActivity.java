...
public class MainActivity extends Activity
{
	 ...
	 String user;
	 int imageIds[] = new int[]{R.drawable.pri_1,R.drawable.pri_2,R.drawable.pri_3};
	 int showall;
	 long last_sync;
	 boolean offline;
	...
	public void onCreate(Bundle savedInstanceState)
	{
	    ...
	    if(offline){
			Toast.makeText(this, "user: "+user+" 离线状态同步已关闭", Toast.LENGTH_LONG).show();
	    }else{
			Toast.makeText(this, "user: "+user, Toast.LENGTH_SHORT).show();
	    }
	    //TABLE user (_id , email , pass , status , last_sync, show_all_tag)
		cursor = db.rawQuery("select * from user where email=?",new String[]{user});
...
//			db.update("user", con2, "email=?", new String[]{user});
	...
	private void sync(){
		...
		if(!offline){
			...
			//上次同步后增加的记录
			Cursor cur1 = db.rawQuery("select name, pri, finish_time, edit_time, create_time, ischeckoff from task where status=? and email=?", new String[]{"1",user});
			...
			cur1.close();
			//上次同步后删除的记录
			Cursor cur2 = db.rawQuery("select create_time from task where status=? and email=?", new String[]{"2",user});
			...
			cur2.close();
			//上次同步后修改的记录
			Cursor cur3 = db.rawQuery("select name, pri, finish_time, edit_time, create_time, ischeckoff from task where status=? and email=?", new String[]{"3",user});
			...
			//Toast.makeText(this, "同步返回数据"+json_syn.toString(), Toast.LENGTH_LONG).show();
			...
			try {
				...
				for(int i = 0;i<jsonarr_delete.length();i++){
					...
					db.delete("task", "create_time=? and email=?", new String[]{json_delete.getString("create_time"),user});
				}
				...
				for(int i = 0;i<jsonarr_update.length();i++){
					...
					db.update("task", cv, "create_time-?<5 and ?-create_time<5 and email=?", new String[]{json_update.getString("create_time"),json_update.getString("create_time"),user});
				}
			} catch (ExecutionException e) {
				// TODO Auto-generated catch block
				Toast.makeText(this, "ExecutionException"+e,Toast.LENGTH_SHORT).show();
				e.printStackTrace();
			} catch (Exception e) {
				// TODO Auto-generated catch block
				Toast.makeText(this, "同步Exception"+e,Toast.LENGTH_SHORT).show();
				e.printStackTrace();
			}
			//置同步状态为0
			db.delete("task", "status=? and email=?", new String[]{"2",user});	
			ContentValues cva = new ContentValues();
			cva.put("status",0);
			db.update("task", cva, "(status=? or status=?) and email=?", new String[]{"1","3",user});
			//更新同步时间
			last_sync = new Date().getTime();
	...
	public void showList()
	{
		...
		if(showall==1){
			cursor = db.rawQuery(
					"select * from task where email=? and status!=2 order by finish_time,pri",new String[]{user});
		}else{
			cursor = db.rawQuery(
					"select * from task where ischeckoff=? and email=? and status!=2 order by finish_time,pri",new String[]{"0",user});
		}
...
//  0 _id  1 name 2 pri  3 finish_time  4 edit_time  5 create_time 6 ischeckoff 7 status  8 email 
...
//			}
		...
		SimpleAdapter simpleAdapter = new SimpleAdapter(this, lis,
			...
			new String[] { "name", "pri" , "finish_time",},
			new int[] { R.id.task, R.id.pri , R.id.date });
		...
		list.setOnItemClickListener(new OnItemClickListener()
		{
			...
			{		
				...
				intent.putExtra("finish_time",lis.get(position).get("finish_time"));
				intent.putExtra("create_time", lis.get(position).get("create_time"));
		...
		switch(item.getItemId())
		{
			case R.id.delete:
				if(Long.parseLong(lis.get(pos).get("create_time"))>last_sync){
					db.delete("task", "_id=? and email=?", new String[]{lis.get(pos).get("_id"),user});
				}else{
					ContentValues con3 = new ContentValues();
					con3.put("status", 2);
					db.update("task", con3, "_id=? and email=?", new String[]{lis.get(pos).get("_id"),user});
				}
				showList();
				break;
			case R.id.checkoff:
				...
				db.update("task", va, "_id=?",new String[]{lis.get(pos).get("_id")});
				if(showall==0){
					showList();
				}
				break;
